...
def executePatternTests(fileNames):
    patternTestScriptLocation = pathlib.Path("patternTestingScripts/executeTest.sh")
    with open(patternTestScriptLocation, "w") as theFile:
        theFile.write("cd /cms/aloeliger/anomalyTriggerWork/comissioningCode/CMSSW_14_1_0_pre5/src/CICADADiagnostics/patternGenerator/patternTesting/ && source env.sh\n\n")
        for fileName in fileNames:
            filePath = pathlib.Path(fileName)
            filePlusSuffix = filePath.stem + filePath.suffix
            fileStem = filePath.stem
            outputFileName = fileStem+'_output.txt'
            theFile.write(f'echo \"Running pattern test for file: {fileName}\"\n')
            theFile.write(f'source pattern_test.sh custom_patterns/{filePlusSuffix} {outputFileName}\n\n')

    ssh_command = f'ssh -t krieger -t \"source {patternTestScriptLocation.absolute()}\"'
...
def main(args):
    ...
    #    executePatternTest(fileName)
    executePatternTests(patternFiles)
